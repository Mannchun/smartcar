...
void uart_getuint8 (UARTn_e uratn, uint8 *ui8)
{
    //等待接收到数据（使能了FIFO后，以下两种方法都可行，都是一样的意思）
    //while( !UART_RCFIFO_REG(UARTN[uratn]) );                                //等待接收到数据数量非0
    //while( (UART_SFIFO_REG(UARTN[uratn]) & UART_SFIFO_RXEMPT_MASK)) ;     //等待接收缓冲区/FIFO 非空的
...
/*!
 *  @brief      发送数字(遇 NULL 停止发送)
 *  @param      UARTn_e       模块号（UART0~UART5）
 *  @param      str         字符串地址
 *  @since      v5.0
 *  Sample usage:   int a=10;    uart_putstr (UART3,a); //    -100 < a <100
 */
void uart_putnum (UARTn_e uratn, const float number)
{
      float num = number;
      uint8 a[6] = {0};
      if(number < 0){
        ...
        a[0] = '+';  
      uint16 j = (uint16)(num*100);
      a[1] = j/1000+0x30;
      a[2] = j%1000/100+0x30;
      a[3] = '.';
      a[4] = j%100/10+0x30;
      a[5] = j%10+0x30;
      uart_putbuff(uratn, a,6);
 }
void uart_putn (UARTn_e uratn, const float number)
{
  float num = number;
      uint8 a[6] = {0};
      if(number < 0){
